...
Проект настроен с полным автоматическим CI/CD пайплайном на GitHub Actions, который запускается при каждом пуше в ветки `main` и `develop`.

### 🔍 **Что делает пайплайн:**

```yaml
1. Code Quality Checks (Windows)     → black, isort, flake8 проверки
2. Testing (Linux с Docker)         → 89/93 тестов с PostgreSQL и Redis
3. Build Docker Images              → Сборка и публикация в GitHub Container Registry
4. Auto Deploy                      → Автоматический деплой на сервер при пуше в main
5. Notifications                    → Уведомления о статусе
✅ Состояние сборки:
https://github.com/Anton742-Tu/HabitFlow-API/actions/workflows/ci.yml/badge.svg

📁 Файл конфигурации:
.github/workflows/ci.yml - полная конфигурация пайплайна

🐳 Docker образы в GitHub Container Registry:
ghcr.io/anton742-tu/habitflow-api:latest - основной образ Django

ghcr.io/anton742-tu/habitflow-api-nginx:latest - образ Nginx

🐳 Production Deployment с Docker
Полная автоматизация деплоя:
```bash
# После пуша в ветку main автоматически:
1. Запускаются все тесты (89/93 проходят, 4 временно пропущены в CI)
2. Собираются Docker образы
3. Образы публикуются в GHCR
4. Происходит деплой на production сервер
5. Выполняются миграции и сбор статики
```
#### Секреты для деплоя (настраиваются в GitHub Secrets):
```yaml
SERVER_IP: "ваш_сервер_ip"
SSH_USER: "ваш_ssh_пользователь"
SSH_PRIVATE_KEY: "ваш_ssh_ключ"
DEPLOY_DIR: "/путь/к/проекту/на/сервере"
```
#### Скрипт деплоя выполняет:
```bash
1. Остановку старых контейнеров
2. Pull обновленных образов из GHCR
3. Запуск docker-compose up -d
4. Выполнение миграций БД
5. Сбор статических файлов
6. Health check приложения
```
## 🔧 Настройка CI/CD для своего сервера
1. Настройте секреты в GitHub:
Перейдите в Settings → Secrets and variables → Actions

Добавьте: SERVER_IP, SSH_USER, SSH_PRIVATE_KEY, DEPLOY_DIR

2. Подготовьте сервер:
```bash
# Создайте директорию
ssh user@server "mkdir -p /opt/habitflow"

# Загрузите docker-compose.yml
scp docker-compose.yml user@server:/opt/habitflow/
```
3. Секрет DEPLOY_DIR должен содержать:
```text
/opt/habitflow  # или ваш путь на сервере
```
## 🧪 Особенности тестирования в CI
### В GitHub Actions тестовая среда включает:

✅ PostgreSQL 15 (тестовая БД: test_habitflow_db)

✅ Redis 7 (для кэширования и Celery)

✅ Автоматические миграции

✅ Пропуск 4 проблемных тестов (работают локально, требуют доработки для CI)

Локально все 93 теста проходят успешно!
## 🧪 Тестирование
```bash
# Запуск всех тестов
